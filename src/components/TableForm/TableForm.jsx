  ...
  showModal = () => {
    ...
    if(this.props.MultiObjectSelector){
      ...
      this.props.dispatch({
        ...
        payload: { multiGroupName, multiObjectType, pageSize: 10, pageNum: 1 ,MasterTableID:this.props.detailData.thisComponentUid,ChildTableData,ChildObject},
      });
    }
  };

  handleOk = e => {  //table类型的数据新增
    ...
    if (this.props.ChildData.length == 0) {
        ...
        HeaderData.fields.map(i => {
          ...
          if (i.text == MultiObjectSelector) {
            ...
            for(let w = 0;w<res.length;w++){
              ...
              arr = arr.replace(reg,n[res[w]]).replace(leftSpace,'').replace(rightSpace,'')  //注：replace不会改变原来的值，arr为处理后的值
            }
      ...
      this.props.dispatch({type:'tableTemplate/childUpdateFields',payload:{params:{
        list,
