...
// @Form.create()
@connect(({ tableTemplate,detailPage,listPage, loading }) => ({
  tableTemplate,
  listPage,
  detailPage,
}))
class DetailButtonGroup extends React.Component {
  // 列表页导入按钮
  tableImport = e => {
    // e表示自定义的按钮
  ...
  // 列表页导出按钮
  tableExport = e => {
    const { searchParams, pageId } = this.props.listPage;
    const div = document.createElement('div');
    document.body.appendChild(div);
    const exportButton = {
      ...this.props,
  ...
  onButtonEvent = e => {
    ...
    this.props.dispatch({
      type: 'detailPage/getTransactionProcess',
      payload: { Buttons: e, selectDate: detailId*1,objectType:ObjectType,PageId},
    });
  };
...
export default withRouter(DetailButtonGroup)