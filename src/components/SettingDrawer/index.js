...
class SettingDrawer extends PureComponent {
  ...
  changeSetting = (key, value) => {
    ...
    if (key === 'layout') {
      ...
      if (value) {
        localStorage.setItem('colorWeak', key)
      } else {
        localStorage.setItem('colorWeak', '')
      }
    } else if (key === 'developerMode') {
      if (value) {
        localStorage.setItem('developerMode', key)
      } else {
    ...
    return (
      <Tooltip title={item.disabled ? item.disabledReason : ''} placement="left">
        <List.Item actions={[action]}>
          <span style={{ opacity: item.disabled ? '0.5' : '', width: '90px', display: 'inline-block' }}>{item.title}</span>
        </List.Item>
      </Tooltip>
    );
  };

  developerChange = (checked) => {
    // window.config.developerMode = checked
    if (checked) {
      localStorage.setItem('developerMode', checked)
    } else {
      localStorage.setItem('developerMode', '')
    }
  ...
  render() {
    ...
    return (
      <Drawer
        ...
        placement="right"
        handler={
          <Button type='primary' className={styles.handle} onClick={this.togglerContent}>
            <Icon
            ...
            <List.Item
              ...
              actions={[
                <Switch
                  style={{ right: '-2rem', position: 'relative' }}
                  size="small"
                  checked={!!localStorage.getItem('colorWeak')}
                  onChange={checked => this.changeSetting('colorWeak', checked)}
                />,
              ]}
            >
              {/* <label>{formatMessage({ id: 'app.setting.weakmode' })}</label> */}
              <label style={{ width: '90px' }}>环保模式</label>
            </List.Item>
            {isSuperUser === 'Yes' && (
              <List.Item
                style={{ width: '100%' }}
                actions={[
                  <Switch
                    style={{ right: '-2rem', position: 'relative' }}
                    size="small"
                    checked={!!localStorage.getItem('developerMode')}
                    onChange={checked => this.changeSetting('developerMode', checked)}
                  />,
                ]}
              >
                {/* <label>{formatMessage({ id: 'app.setting.weakmode' })}</label> */}
                <label style={{ width: '90px' }}>开发者模式</label>
              </List.Item>
            )}
